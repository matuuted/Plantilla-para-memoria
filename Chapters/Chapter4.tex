...
pruebas (protoboard) mediante cables de conexión tipo jumper,
...
\begin{itemize}
    ...
    \item Terminal serie (software), utilizada para registrar los valores
...
NEMA~17 con el driver A4988, alimentado a 12~V. Con el analizador lógico
...
frecuencia configurada por software. Las señales GPIO DIR y ENABLE se
...
del driver, y la variación de la frecuencia de la señal STEP para
comprobar el rango de velocidades.

El motor respondió correctamente en todo el rango de velocidades ensayado.
No se detectó pérdida de pasos ni calentamiento excesivo del driver
...
lectura del DS3231. Se observa la dirección del dispositivo (\texttt{0x68}) en las
...
implementado por software resultó suficiente para suprimirlo.
...
En esta sección se presentan las pruebas de las funciones de software:
el comportamiento de la máquina de estados, la gestión de tareas concurrentes
bajo FreeRTOS y el protocolo de comunicación, junto con la verificación del timing de los mensajes de telemetría.
...
\subsection{Protocolo de comunicación y timing de telemetría}

Se verificó el protocolo de comunicación \texttt{JSON} entre el microcontrolador y la
...
\begin{figure}[H]
    ...
    \caption{Registro de la comunicación \texttt{JSON} entre la aplicación y el microcontrolador.}
    \label{fig:prueba_bt}
\end{figure}
...
módulos interconectados y el firmware integrado con FreeRTOS. Se describen
...
sistema completo sobre la protoboard, con todos los componentes
...
\begin{table}[H]
    ...
    \begin{tabular}{p{5.5cm} p{5.5cm} c}
        ...
        Nuevos horarios almacenados en flash y activos &
        Conforme \\
...
se conservó ante cortes de energía, y la comunicación \texttt{JSON} no presentó errores
...
\begin{itemize}
    ...
    \item El ensamblado sobre protoboard con cables jumper
